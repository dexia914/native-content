...
说明：
- `softpost`：前台运行，关闭当前会话后服务会结束
- `softpost-start`：后台启动，会写 PID 到 `run/softpost.pid`
- `softpost-stop`：读取 `run/softpost.pid` 并结束对应 Web 进程

如果你还想继续使用旧的命令行方式，改用：

```bash
softpost-cli --help
softpost-cli generate --topic "看短剧赚钱" --audience "宝妈,大学生" --core "玩一玩就能赚钱、看短剧赚钱"
```
...
- `XHS_COMPLIANCE_MODE`：是否启用小红书合规模式，默认 `true`
- `MYSQL_URL`：MySQL 连接串，用于保存生成记录并驱动 Web 页“最近生成的产物”列表
- `FONT_PATH`：可选。若系统缺少中文字体，可手动指定字体文件路径

Linux/Ubuntu 说明：
- Ubuntu 24 不等于一定是纯 headless，但云服务器场景通常是 headless 环境
- `softpost-cli generate` 在 Ubuntu 上通常可运行，但建议安装中文字体；若未安装，封面中文可能显示异常
- 当前代码已自动适配 Windows 和 Ubuntu 常见中文字体路径
- 若仍找不到合适字体，请在 `.env` 中显式设置 `FONT_PATH`
- `softpost-cli publish` 在 Ubuntu 上也可以运行，但若服务器没有桌面环境，`softpost auth` 这类需要人工登录的步骤通常要借助图形界面、远程桌面或虚拟显示
...
- 生成软文与封面
- 浏览最近生成的产物（从 MySQL 读取）
- 检查 `.auth/xiaohongshu.json` 登录态剩余有效期
- 从已有 `outputs/...` 目录发起发布
- 查看最近日志，定位成功状态和错误原因

说明：
- `softpost-cli auth` 这类需要人工登录浏览器的流程，仍建议使用 `softpost-cli auth`
- Web 页面本身不直接托管登录动作
- 每次生成后，记录会自动写入 MySQL 表 `generated_posts`
- Web 服务会把操作日志写入 `logs/web.log`
- Web 首页的“最近日志”面板会展示最近日志内容
...
```env
XHS_COMPLIANCE_MODE=true
```

开启后会做两层处理：
- 约束模型生成更像“真实体验分享”，减少营销和引流表达
- 对生成结果做本地清洗，弱化高风险词，如“赚钱 / 搞钱 / 变现 / 月入 / 发你入口 / 私信我”

目标不是“绕过审核”，而是尽量生成更符合平台真实分享语境的内容，减少明显违规表达。
...
```bash
softpost-cli generate --topic "居家手工副业" --audience "宝妈" --core "不出门也能做、带娃空档可操作"
```

```bash
softpost-cli generate --topic "宿舍副业" --audience "大学生" --core "低门槛、零碎时间可做、赚生活费"
```
...
```bash
softpost-cli publish --artifact-dir "outputs/20260101_120000_看短剧赚钱"
```

如果你只执行上面这条命令：
- 脚本会尝试打开发布页、上传图片、填写标题和正文
- 但不会自动点击最终“发布”按钮
- 流程结束后浏览器会关闭

如果你希望脚本继续尝试自动点击“发布”，请使用：

```bash
softpost-cli publish --artifact-dir "outputs/20260101_120000_看短剧赚钱" --auto-submit
```

`--auto-submit` 的作用：
- 自动尝试点击“发布”
- 如有“确认发布”之类弹窗，也会继续尝试点击确认
...
- `app/cli.py`: 旧命令行入口（generate / auth / auth-status / publish）。
- `app/web.py`: FastAPI 服务与 Web 控制台。
